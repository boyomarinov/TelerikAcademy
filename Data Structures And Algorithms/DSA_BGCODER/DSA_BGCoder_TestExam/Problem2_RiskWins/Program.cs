...
namespace Problem2
{
    ...
    {
        private static int[] superPowers = new int[] { 1, 10, 100, 1000, 10000, 100000 };
        ...
        public static int[] GenerateNext(int num)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                int currentDigit = (num / superPowers[i]) % 10;
                result[resultIndex++] = num + superPowers[i] * (NextDigit(currentDigit, true) - currentDigit);
                result[resultIndex++] = num + superPowers[i] * (NextDigit(currentDigit, false) - currentDigit);
            }
